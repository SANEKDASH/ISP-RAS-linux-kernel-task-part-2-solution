...
Так же syzkaller'ом были сгенерированы репродьюсеры этого варнинга, то есть программы, при выполнении которых происходит наша проблема.
На код репродьюсеров можно посмотреть [здесь](ССЫЛКА).

Моя задача на данный момент - найти причину, по которой срабатывает этот варнинг.

# Анализ варнинга
В нашем случае syzkaller сгенерировал репродьюсеров. Этот факт сильно облегчает дальнейшие размышления.
...
```
Зная это, определим, какие флаги передаются в этот системный вызов, чтобы определить его дальнейшее поведение:
По агрументам системных вызовов видно, что битовое поле с флагами - 0x32.
Отсюда определим флаги:

```
flags = 0x32 = 0b00110010
                   ||  |
                   ||  +--> MAP_PRIVATE		   
		           |+-----> MAP_FIXED
		           +------> MAP_ANONYMOUS
...
Так же по аргументам можем видеть, что в первом и последнем системном вызове __prot__ = 0 = PROT_NONE.
Во втором: __prot__ = PROT_EXEC | PROT_READ | PROT_WRITE | PROTE_NONE.